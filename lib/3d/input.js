...
/** True when a key event belongs to a button, link, form field or [data-ui]
 *  element — those own their own keyboard handling, so the world stays out. */
function fromInteractiveUI(e) {
  const target =
    e.target instanceof Element ? e.target : document.activeElement;
  if (!target || !(target instanceof Element)) return false;
  return (
    INTERACTIVE_TAGS.includes(target.tagName) ||
    Boolean(target.closest("[data-ui]"))
  );
}
...
export function attachControls(element, isBlocked) {
  ...
  const onKeyDown = (e) => {
    ...
    if (blocked() || fromInteractiveUI(e)) return;
  ...
  // keyup stays unguarded on purpose: a key pressed in the world but released
  // over UI must still clear, or it would stay held forever.
